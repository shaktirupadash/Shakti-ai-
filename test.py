...
# Page config
st.set_page_config(
    page_title="Shakti-Gemini Unit Test Generator",
    page_icon="⚡",
    layout="wide"
)
...
def create_clean_document_with_images(test_data, frame_files, video_duration=0, video_name="test_video"):
    ...
    doc.add_paragraph()
    doc.add_paragraph('_' * 80)
    footer = doc.add_paragraph('Generated by Shakti-Gemini Unit Test Generator')
...
st.markdown('<p class="subtitle">Clean Unit Test Document Generator</p>', unsafe_allow_html=True)

# Sidebar
with st.sidebar:
    st.title("⚙️ Control Center")

    # Get API Key from Streamlit Secrets (CHANGED THIS SECTION)
    try:
...
# Main Area
if st.session_state.current_video_file:
    
    if st.button("📝 GENERATE CLEAN UNIT TEST DOCUMENT", type="primary", use_container_width=True):
...
st.divider()
st.caption(f"⚡ Shakti-Gemini | {datetime.now().strftime('%I:%M %p')}")
